...
static uint32_t dfsdm_getreg(FAR struct stm32_dev_s *priv, int offset);
static void     dfsdm_putreg(FAR struct stm32_dev_s *priv, int offset,
                             uint32_t value);
...
static void dfsdm_rccreset(FAR struct stm32_dev_s *priv, bool reset);
static void dfsdm_enable(FAR struct stm32_dev_s *priv);
static int  dfsdm_set_ch(FAR struct adc_dev_s *dev, uint8_t ch);

#ifdef DFSDM_HAVE_TIMER
static void dfsdm_timstart(FAR struct stm32_dev_s *priv, bool enable);
static int  dfsdm_timinit(FAR struct stm32_dev_s *priv);
#endif
...
static void dfsdm_startconv(FAR struct stm32_dev_s *priv, bool enable);

/* Interrupt Handler */

static int dfsdm_interrupt(FAR struct adc_dev_s *dev, uint32_t regval);
#if defined(CONFIG_STM32L4_DFSDM1_FLT0)
static int dfsdm_flt0_interrupt(int irq, FAR void *context, FAR void *arg);
#endif
...
static int  dfsdm_bind(FAR struct adc_dev_s *dev,
